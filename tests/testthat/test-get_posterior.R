...
test_that("Extracted tables are correct when proper parameter names are given", {
    # TODO: test models with formula_* / formula_*_shared formulae.
...
#              "alpha_share", "beta_shared", "gamma_shared",
    ...
    for (i in seq_along(lpar)) {
        test <- get_post_summary(fit, lpar[i])
        attr(test, "dimension") <- NULL
        attr(test, "label") <- NULL
        ans <- fit@fit$summary[grep(paste0(lpar[i], "\\["), rownames(fit@fit$summary)), ]
